...
void updateTimezone() {
  Serial.println("[DEBUG] Chiamata: updateTimezone()");
...
void initSNTP(bool waitForSync) {
  Serial.println("[DEBUG] Chiamata: initSNTP()");
...
void setup() {
  ...
  // Pulsante fisico per avvio "FUNERALE"
  pinMode(FUNERAL_BUTTON_PIN, INPUT_PULLUP);
...
void loop() {
  ...
  // Gestione pulsante fisico FUNERALE (debounce + cooldown)
  {
    ...
    bool state = digitalRead(FUNERAL_BUTTON_PIN);
    if (lastState != state) {
      lastState = state;
      if (!state) { // pressione (LOW)
        uint32_t now = millis();
...
void debugScheduleCheck() {
    struct tm ti;
    if (!getLocalTm(ti)) {
        Serial.println("[DEBUG] debugScheduleCheck: impossibile ottenere ora locale");
        return;
    }
